...
// Function to start Nginx for production use
function startNginx() {
  try {
    console.log('Starting Nginx...');
    // Copy nginx.conf to the correct location
    fs.copyFileSync('nginx.conf', '/etc/nginx/conf.d/default.conf');
    ...
    // Start Express server as fallback
    startExpressServer();
  }
}
...
function startExpressServer() {
  ...
  // URL rewriting middleware - must be added BEFORE static file middleware
  app.use((req, res, next) => {
    // Log all incoming requests for debugging
    console.log(`[URL Rewriter] Original URL: ${req.url}`);
    
    // Clean URLs mapping to HTML files (add more as needed)
    const urlMap = {
      '/': '/html/index.html',
      '/shop': '/html/shop.html',
    ...
    if (urlMap[req.url]) {
      console.log(`[URL Rewriter] Rewriting ${req.url} to ${urlMap[req.url]}`);
...
if (isProduction && fs.existsSync('/etc/nginx')) {
  startNginx();
} else {
  startExpressServer();
} 